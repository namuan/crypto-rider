...
class OrderDataStore(Thread, BaseContainer):
    ...
    def save_trade_order(self, event):
        trade_order = self.last_trade_order(event.get("market"))
    ...
    def force_close(self, market, dt_since, dt_to):
        trade_order = self.last_trade_order(market)
        if trade_order and trade_order.is_open:
            market_data_df = self.lookup_object("market_data_store").fetch_data(
    ...
    def last_trade_order(self, market):
