...
class RsiZoneStrategy(BaseStrategy):
    ...
    def get_additional_plots(self, market, dt_since, dt_to):
        ...
        return [
            mpf.make_addplot(df[self.trend_indicator][dt_since:dt_end], linestyle="dashed"),
            mpf.make_addplot(df[self.rsi_indicator][dt_since:dt_end], linestyle="dashed", panel=2),
        ]
